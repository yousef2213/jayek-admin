...
export default function DeActive() {
    const { ResturantsDeActivation = [], DelResturants, Activation } = useContext(JayAdminContext);

    if(ResturantsDeActivation.length === 0) {
        return (
            <div>
                <div className="w-100 px-0 mx-0">
                    ...
                    <h4 className="text-center font-main py-4 text-capitalize">There are no inactive offers</h4>
                </div>
            </div>
        )
    }
            ...
            <div className="all-menu w-100 px-0 mx-0">
                <table className="table">
                    ...
                    <tbody className="par_cat">
                        {ResturantsDeActivation.map( ({id,name,type,email,phone}) => {
                            return (
                                <tr key={id}>
                                    ...
                                    <td scope="row" className="font-main text-capitalize head-table text-center" style={{fontSize:"13px",verticalAlign: "middle"}}>
                                        <div className="div-input d-flex justify-content-around px-0 mx-0">
                                            <button className="baged btn btn-success-new btn-sm" style={{borderRadius:"10px"}} onClick={() => Activation(id)} >Reactivate</button>
